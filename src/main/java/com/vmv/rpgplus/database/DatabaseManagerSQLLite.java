...
public class DatabaseManagerSQLLite extends DatabaseManager implements Database {

    public DatabaseManagerSQLLite(Plugin plugin, String fileName, File location) {
        super(plugin, fileName, location);
        createTables();
    }
    ...
    public void createTables() {
        ...
        executeSQL("CREATE TABLE IF NOT EXISTS [player_allocations] (uuid VARCHAR PRIMARY KEY NOT NULL)", false, false); //must be created before altered
    ...
    public void newPlayer(UUID uuid) {
        ...
        executeSQL("INSERT OR IGNORE INTO player_experience(uuid) VALUES('" + uuid + "')", true);
        executeSQL("INSERT OR IGNORE INTO player_settings(uuid) VALUES('" + uuid + "')", true);
        executeSQL("INSERT OR IGNORE INTO player_allocations(uuid) VALUES('" + uuid + "')", true);
    ...
    /**
     * Saves data stored in memory that would usually be saved periodically, called when a user leaves
     */
    @Override
